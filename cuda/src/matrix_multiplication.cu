...
void CpuMatrixVector(float *matrix_A, float *matrix_B, float *matrix_C, int N, int K, int M) {
    for (int i = 0; i < N; ++i) 
    {
        for (int j = 0; j < M; ++j) 
        {
            ...
            for (int h = 0; h < K; ++h) 
            {
                tmp += matrix_A[i * K + h] * matrix_B[h * M + j];
            }
            matrix_C[i * M + j] += tmp;
        }
    }
...
__global__ void gpuMatrixVector(float* A, float* B, float* C, int n, int k, int m)
{
  ...
  if (row < n && col < m) {
    ...
    for (int i = 0; i < k; ++i) {
      //if(row == 3 && col == 1)
      //  printf("PRE: %d %d     %d %d %d     %d      %d %d %d ----- %f %f %f\n",row, col, n, k, m, i,row * m + col,row * k + i,i * m + col,C[row * n + col], A[row * k + i], B[i * m + col]);
      sum += A[row * k + i] * B[i * m + col];
      //if(row == 3 && col == 1)
        //printf("POST: %d %d %d ----- %f %f %f\n",row * n + col,row * k + i,i * m + col,C[row * n + col], A[row * k + i], B[i * m + col]);
    }
    C[row * m + col] += sum;
