...
void generate_block_cyclic_distribution(
    ...
    psizes[0] = process_grid_rows; /* no. of processes in vertical dimension of process grid */
    psizes[1] = process_grid_cols; /* no. of processes in horizontal dimension of process grid */

    MPI_Type_create_darray(process_grid_rows * process_grid_cols, my_rank, 2, gsizes, distribs, dargs, psizes, MPI_ORDER_C, MPI_FLOAT, &filetype);
    MPI_Type_commit(&filetype);
    ...
    matrix_size = compute_submatrix_dimension(rows, cols, block_rows, block_cols, process_grid_rows, process_grid_cols, my_rank);
 ...
 * @param argc Number of arguments when launching the program to pass to MPI_init
 * @param argv Arguments when launching the program to pass to MPI_init
*/
void compute_process_parallel_matrix_by_matrix_multiplication(
    ...
    int number_of_processes,process_grid_rows, process_grid_cols, my_rank;
    MPI_Comm comm;
    ...
    generate_block_cyclic_distribution(matrix_A_file_name, cols, rows, BLOCK_ROWS, BLOCK_COLS,process_grid_rows,process_grid_cols,my_rank,comm);
